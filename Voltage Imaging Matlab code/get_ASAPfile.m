function [asapfile,asapfiles,videonumber] = get_ASAPfile(ID,day,session)

drive = pwd;
drive = drive(1);
asapfiles = dir(fullfile([drive,':/ASAP/'],ID,day,'*.tif'));        % Find all tif files WITHIN THE SPECIFIC DIRECTORY (not subfolders)
...
for s = 1:length(asapfiles)
    asapfiles{s} =  fullfile([drive,':/ASAP/'],ID,day,asapfiles{s}); % Add full directory
end
