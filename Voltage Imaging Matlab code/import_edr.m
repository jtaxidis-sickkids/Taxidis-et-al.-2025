...
%     s = strtrim(s);
    ...
    k = strfind(s,'=');
    if ~(isempty(k)) & isempty(strfind(s,'ÿ')) & ~isempty(s(k+1:length(s))) % SECOND AND THIRD CONDITION ADDED BY ME DUE TO PROBLEMATIC EDR FILE
        key = s(1:k-1);
        if strncmp(key,'YN',2) | strncmp(key,'YU',2) | strcmp(key,'TU') | strcmp(key,'ID') | strcmp(key,'BAK') | strcmp(key,'WCPFNAM')
            h.(key) = s(k+1:length(s));
        else
            h.(key) = str2double(s(k+1:length(s)));
        end
    end
end
